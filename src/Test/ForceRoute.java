  ...
  public static void main(String[] args) throws Exception {
	...
	
	 {
		 ...
		 for (int row = 1; row <= totalNoOfRows; row++) 
		 {
			 ...
			 String Runmode= Utility7.getCellData(sFileName,sSheetName,row, 8);
			
			 if(Runmode.equals("Yes")) {
				 String Type=Utility7.getCellData(sFileName,sSheetName,row, 7);
				...
				if(elementdate!=null){
					...
					for (int k = 0; k < links6.size(); k++) {
						...
						System.out.println("Worflow Type is:" + text3);
												...
												// given in excel sheet to navigate to Active date or historical dates
						if (text3.equalsIgnoreCase(Utility7.getCellData(sFileName,sSheetName,row, 3))) {
							element3.click();
							break;
						}
													...
													
					{
						...
						try {
							...
							Thread.sleep(5000);
							//provide the custom folder path
							String textF = Utility7.getCellData(sFileName,str,0, 3);
							StringSelection stringSelection = new StringSelection(textF);
							Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
							clipboard.setContents(stringSelection, stringSelection);
							
							//provide the custom file name
							String textb = Utility7.getCellData(sFileName,str,Arow, 2);
							String strs="\\";
						...
						driver.findElement(By.xpath("//input[@value='Reset']")).click();}
					int totalNoOfRows2 = Utility7.rowcount(str);
...
try {
			...
			try {
				...
				if(elementdate!=null){
					...
					for (int j = 0; j < links2.size(); j++) {
						...
						System.out.println("Project name is:" + text2);
						// given in excel sheet to navigate to Active date or historical dates
						if (text2.equalsIgnoreCase(Utility7.getCellData(sFileName,sSheetName,row, 4))) {
			...
			subview.click();
													...
													
					{
						...
						for (int l = 0; l < days.size(); l++) {
							...
							String selectday=Utility7.getCellData(sFileName,str,0, 3);
						...
						try {
							WebElement e4 = driver.findElement(By.xpath("//input[@id='filter_Text_value2']"));
							e4.clear();
							e4.sendKeys(Utility7.getCellData(sFileName,str,Arow, 0));
						} catch (Exception e) {
							System.out.println("unable to enter data from excel on to notes text box");
						}
	...
	for (int i = 0; i < links.size(); i++) {
					...
					try {
						...
						for (int p = 0; p < links5.size(); p++) {
							...
							System.out.println("Project name is:" + text5);
														// given in excel sheet to navigate to Active date or historical dates
							if (text5.equalsIgnoreCase(Utility7.getCellData(sFileName,sSheetName,row, 4))) {
									...
									if (select1 != null) {
										...
										for (int s = 0; s < isize; s++) {
											...
											if (sValue.equalsIgnoreCase("Force Route")) {
												values.selectByVisibleText(sValue);
												driver.findElement(By.xpath("//*[@id='RouteWorkflow']/div[3]/input[2]")).click();
												System.out.println("act# got Force Routed succesfully "+Utility7.getCellData(sFileName,str,Arow, 3));
												driver.findElement(By.xpath("//input[@value='Reset']")).click();
												break;
											}
											else if (sValue.equalsIgnoreCase("Cancel")) {
												values.selectByVisibleText(sValue);
												System.out.println("act# got Cancelled succesfully "+Utility7.getCellData(sFileName,str,Arow, 3));
												driver.findElement(By.xpath("//input[@value='Reset']")).click();
												break;
											}
										}
		...
		/*{
			...
			//WebDriver driver = new ChromeDriver(); 
			//driver.manage().window().maximize();
			//driver.manage().Force RouteAllCookies();
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);

			//loading the excel file
			try {
			Utility7.setExcelFile(sFileName1, sSheetName1);}
			catch (Exception e){System.out.println("invalid excel file");
			}

			//navigating to Viaduct UAT instance from excel file
			try {
				driver.get(Utility7.getCellData(sFileName1, sSheetName1, 1, 8));
			} catch (Exception e3) {
				e3.printStackTrace();
			}
			try {
			...
			try {
			driver.findElement(By.name("SWEUserName")).sendKeys(Utility7.getCellData(sFileName1, sSheetName1, 1, 6));
			driver.findElement(By.name("SWEPassword")).sendKeys(Utility7.getCellData(sFileName1, sSheetName1, 1, 7));
			...
			for ( row = 1; row <= totalNoOfRows; row++) 
			{
				...
				int count=0;
				try {
					sRunMode1=Utility7.getCellData(sFileName1, sSheetName1, row, 2);
				} catch (Exception e2) {
					e2.printStackTrace();
				}
			...
			String Cname = null;
			try {
				Cname = Utility7.getCellData(sFileName1, sSheetName1, row, 0);
			} catch (Exception e1) {
				e1.printStackTrace();
			}

			//driver.findElement(By.xpath("//table[@id='s_1_l']/tbody/tr[2]/td[2]/input")).sendKeys(Utility7.getCellData(sFileName, sSheetName, row, 0));
			try {
			...
			try {
			System.out.println(Utility7.getCellData(sFileName1,sSheetName1,row, 1));
			String str = Utility7.getCellData(sFileName1,sSheetName1,row, 1);
			System.out.println("account sheetname"+str);
			int totalNoOfRows1 = Utility7.rowcount(str);
			...
			String S1="*";
			String ActName=Utility7.getCellData(sFileName1,str,Arow, 0);
			
			//driver.findElement(By.xpath("//input[@id='1_Account_Number']")).sendKeys(Utility7.getCellData(sFileName,str,Arow, 0));}
			...
			promptAlert.sendKeys(Utility7.getCellData(sFileName1,sSheetName1,row, 3));
			...
			if(count>=1) {
				...
				try {
					Utility7.setCellData("Fail", row, 5);
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				try {
					Utility7.setCellData("Pass", row, 5);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
				...
				try {
				String S="*";
				String Cname=Utility7.getCellData(sFileName1, sSheetName1, row, 0);
				...
				e1.sendKeys(Utility7.getCellData(sFileName1,str,Arow, 0));
